...
import useFetchMyPresentations from "./Book Presentation/FetchMyPresentations"
...
function BookPresentationModal({
  ...
  const { myPresentations, localChildData, error } = useFetchMyPresentations();  // <-- Use the custom hook here

  // Any other useEffects that depend on `canBookForChild`
  useEffect(() => {
    console.log(
      "canBookCampus:",
      canBookCampus,
  ...
  const handleCancelPresentation = async (presentationId, timeSlotIndex) => {
    if (
      ...
      if (response.status === 200) {
        useFetchMyPresentationsfetchMyPresentations(); // reload presentations
  ...
  useEffect(() => {
    const fetchCanBookForTestGrade = async () => {
      ...
      try {
        ...
        console.error("Error fetching canBook status:", err);
      }
    };
  ...
  return (
    <div className="book-presentation-modal-new">
      <FetchMyPresentations 
        setMyPresentations={setMyPresentations} 
        setLocalChildData={setLocalChildData}
        setError={setError}
      />
