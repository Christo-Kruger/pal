...
function BookPresentationModal({
    ...
    const fetchCanBookForTestGrade = async () => {
      ...
      try {
        ...
        if (response.data && response.data.length > 0) {
          console.log("Setting canBookForChild to:", response.data[0].canBook);
  
          // Set canBookForChild based on the API response
          setCanBookForChild(response.data[0].canBook === true);
        }
      } catch (err) {
  ...
  useEffect(() => {
    console.log("canBookCampus:", canBookCampus, "canBookForChild:", canBookForChild);
  ...
  const filteredMyPresentations = myPresentations
    ...
    console.log("canBookCampus:", canBookCampus, "canBookForChild:", canBookForChild);  // <-- Add this line
