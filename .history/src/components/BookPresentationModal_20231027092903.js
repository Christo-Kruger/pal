...
function BookPresentationModal({
  ...
  const [isLoading, setIsLoading] = useState(true); // Initialize to true to indicate loading
  const [pendingFetches, setPendingFetches] = useState(3); // Initialize to 3 because we have 3 fetch operations
  ...
  const fetchMyPresentations = async () => {
    ...
    try {
      ...
      setError("Error fetching my presentations");
      setPendingFetches((prev) => prev - 1); // Decrement the pending fetches
    } finally {
      sconsole.log("Before decrementing:", pendingFetches);
