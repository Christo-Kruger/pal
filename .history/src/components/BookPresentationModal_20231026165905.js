...
function BookPresentationModal({
  ...
  useEffect(() => {
    const fetchCanBookForTestGrade = async () => {
      ...
      try {
        console.log("Child Test Grade:", childData.testGrade); // Add this line
        
        const response = await axios.get(
          `${backendURL}/api/canBook/${childData.testGrade}`
        );
  
        console.log("API Response:", response.data); // Existing log
  
        if (response.data && response.data.length > 0) {
          console.log("canBook Value:", response.data[0].canBook); // Add this line
