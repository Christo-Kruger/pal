...
const handleAgeGroupChange = (index, event) => {
  ...
  const handleAgeGroupCanBookChange = async (id, checked) => {
    try {
      const response = await axios.patch(`${backendURL}/api/campus/agegroup/${id}`, {
        canBook: checked
      });
      // Update your state or handle the response as you see fit
      const updatedCampus = response.data;
      // Assuming you would update your rows here, this is just an example
