...
export default function CanBook() {
  ...
  const updateRow = async () => {
    ...
    if (selectedRows.length > 0) {
      ...
      const response = await axios.put(`${backendURL}/api/canBook/${idToUpdate}`, recordToUpdate);
  ...
  const deleteRow = async () => {
    ...
    if (selectedRows.length > 0) {
      ...
      await axios.delete(`${backendURL}/api/canBook/${idToDelete}`);
