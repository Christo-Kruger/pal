...
export default function Campus() {
  ...
  const [campusIdToDelete, setCampusIdToDelete] = useState(null);
  const [newAgeGroups, setNewAgeGroups] = useState([
    { ageGroup: "", canBook: false },
  ]);
  ...
  const handleConfirmDelete = () => {
    axios
      .delete(`${backendURL}/api/campus/${campusIdToDelete}`)
      .then(async () => {
        const response = await axios.get(`${backendURL}/api/campus`);
        setRows(response.data);
  ...
  const handleRowClick = (params) => {
    const ageGroupsWithCampusId = (params.row.ageGroups || []).map((ag) => ({
      ...ag,
      campusId: params.row._id,
    }));
  ...
  const handleAgeGroupCanBookChange = async (campusId, ageGroup, checked) => {
    try {
      ...
      // Update the rows
      const updatedRows = rows.map(row => row._id === updatedCampus._id ? updatedCampus : row);
      setRows(updatedRows);
  
      // Update the selected age groups
      const updatedSelectedAgeGroups = selectedAgeGroups.map(ag =>
        ag.ageGroup === ageGroup ? { ...ag, canBook: checked } : ag
      );
      setSelectedAgeGroups(updatedSelectedAgeGroups);
  ...
  const handleCanBookChange = async (id, value) => {
    try {
      await axios.patch(`${backendURL}/api/campus/canbook/${id}`, { canBook: value });
  
      // Update just the changed row in the local state
        ...
        <DialogContent>
          ...
          {newAgeGroups.map((inputField, index) => (
            <div key={index}>
              ...
              {index !== 0 && (
                <IconButton
                  onClick={() => removeNewAgeGroup(index)}
                  color="secondary"
                >
                  <RemoveCircleIcon />
                </IconButton>
