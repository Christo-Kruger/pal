...
import { Checkbox, IconButton, Dialog, DialogActions, DialogContent, DialogContentText, DialogTitle, Button, TextField } from '@mui/material';
      ...
      renderCell: (params) => (
        <Checkbox
          ...
          onChange={async () => {
            ...
            await axios.put(`${backendURL}/api/canBook/${params.id}`, { canBook: newValue });
            setRows(rows.map(row => row.id === params.id ? { ...row, canBook: newValue } : row));
          }}
        />
      )
    },
