            ...
            {presentation.timeSlots ? (
              presentation.timeSlots.map((slot) => (
                <div
                  className="time-slot"
                  onClick={() => {
                    if (Array.isArray(slot.attendees) && slot.attendees.length >= slot.maxAttendees) {
                  ...
                  <div className="slot-info">
                    <button
                      className={`slotButton${
                        (Array.isArray(slot.attendees) && slot.attendees.length >= slot.maxAttendees) || 
                        (Array.isArray(slot.attendees) && slot.attendees.includes(getUserId()))
                          ? ' disabled-slot'
                          : ''
                      }`}
                      onClick={(e) => {
