  ...
  {presentation.timeSlots ? (
    presentation.timeSlots.map((slot) => (
      <div
        className="time-slot"
        onClick={() => {
          if (slot.attendeeCount >= slot.maxAttendees) { // Changed this line
        ...
        <div className="slot-info">
          <button
            className={`slotButton${
              slot.attendeeCount >= slot.maxAttendees || slot.attendees.includes(getUserId()) // Changed this line
                ? ' disabled-slot'
                : ''
            }`}
            onClick={(e) => {
          ...
          >
            {slot.attendeeCount >= slot.maxAttendees  // Changed this line
              ? '예약마감'
              : '예약'}
          </button>
        </div>
