...
function BookPresentationModal({
  ...
  // New useEffect to check booking eligibility for child's age group
  useEffect(() => {
    const fetchCanBookForAgeGroup = async () => {
      const backendURL = process.env.REACT_APP_BACKEND_URL;
      try {
        const response = await axios.get(`${backendURL}/api/canBook/${childData.ageGroup}`);
        if (response.data && response.data.length > 0) {
          setCanBookForChild(response.data[0].canBook); // assuming canBook is a boolean field in your model
        }
      } catch (err) {
