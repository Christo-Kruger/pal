...
if (response.status === 200) {
  ...
  if (!Array.isArray(updatedPresentations[presentationIndex].timeSlots[slotIndex].attendees)) {
    console.log("Attendees not found in updated array. Creating an empty array.");
    updatedPresentations[presentationIndex].timeSlots[slotIndex].attendees = [];
  }
            ...
            <>
              {canBook ? ( // Check both campus and child age group eligibility
                ...
                <p style={{ flex: 1, textAlign: "center" }}>
                  You are not eligible to book right now.
                </p>
              )}
            </>
          )}
