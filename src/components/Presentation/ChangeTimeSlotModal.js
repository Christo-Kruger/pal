...
function ChangeTimeSlotModal({
  ...
  const handleChangeTimeSlot = async () => {
    ...
    const newTime = timeSlots.find(
      (slot) => slot._id === selectedTimeSlot
    )?.startTime;

    const confirmChange = window.confirm(
      `변경 시 실시간 예약에 따라 재변경이 어려울 수 있습니다. 시간 변경하시겠습니까? ${new Date(
        oldTime
      ).toLocaleTimeString()} -> ${new Date(newTime).toLocaleTimeString()}?`
    );
