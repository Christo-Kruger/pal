...
const MyTestBooking = ({ bookings, setBookings, childData }) => {
  ...
  const [oldTimeSlot, setOldTimeSlotId] = useState(null);

  const handleTimeSlotChange = (bookingId) => {
    const booking = bookings.find((b) => b._id === bookingId);
    const oldTimeSlot = booking?.testSlot?.timeSlots?.[0];
    setOldTimeSlotId(oldTimeSlot._id);
        ...
        >
          <CardContent>
            ...
            <Typography variant="body1">
              시험등급: {booking.child.testGrade}
            </Typography>
            <Typography variant="body1">
              교정: {booking?.testSlot?.campus}
            </Typography>
            <Typography variant="body1">
              날짜:{" "}
              {new Date(booking?.testSlot?.date).toLocaleDateString("ko-KR", {
                weekday: "long",
                year: "numeric",
                month: "long",
                day: "numeric",
            ...
            <div>
              ...
              <button
                style={{ width: "150px" }}
                onClick={() => handleTimeSlotChange(booking._id)}
              >
                예약시간변경
              </button>
            </div>
      ...
      <Dialog open={isModalVisible} onClose={handleCloseModal}>
        ...
        <DialogContent>
          <ChangeTest
            ...
            oldTimeSlot={oldTimeSlot} // pass oldTimeSlotId here
